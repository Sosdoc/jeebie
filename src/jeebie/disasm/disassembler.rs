...
impl Disassembler {
    ...
    fn get_opcode_name(&mut self, data: &[u8]) -> Result<String, String> {
        match data.get(self.pc) {
            Some(&0xCB) => {
                self.pc = self.pc.wrapping_add(1);

                if let Some(second_byte) = data.get(self.pc) {
    ...
    /// values filled in.
    pub fn get_instruction_str(&mut self, data: &[u8]) -> Result<String, String> {
        let pc = self.pc;
        ...
        let low = data.get(self.pc.wrapping_add(1)).map(|x| *x as u16);
        let high = data.get(self.pc.wrapping_add(2)).map(|x| (*x as u16) << 8);
        self.pc = self.pc.wrapping_add(1);
